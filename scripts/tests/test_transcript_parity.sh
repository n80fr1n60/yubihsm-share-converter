#!/usr/bin/env bash
# R12-01 / R12-02 / R13-05 (Phase E): anti-R10-L2 methodology gate.
#
...
#
# R13-05 (Phase E) uplift: the fixture tree is now per-SDK-version. We
# iterate over every `v_*` subdirectory and run the full parser-audit
# suite against each version's transcripts. A new SDK version is added
# by creating a new `v_X.Y.Z/` subdirectory with the 6-file fixture
# set; the gate picks it up automatically. The post-R13-E posture is
# format-drift detection across SDK versions (e.g. 2.7.1 pre-hotfix
# shape, 2.7.3 R12-B baseline, 2.7.4 hypothetical trailing-fields
# extension).
#
...
FIXTURES_ROOT="${REPO_ROOT}/tests/fixtures/device-transcripts"
...
# Sanity-check the fixtures-root + production scripts.
[ -d "${FIXTURES_ROOT}" ] || { echo "FAIL: fixtures root missing: ${FIXTURES_ROOT}" >&2; exit 1; }
for f in "${STAGE_SH}" "${EXTRACT_SH}" "${VERIFY_SH}" "${DRIVER_PY}"; do
    [ -r "${f}" ] || { echo "FAIL: production script missing: ${f}" >&2; exit 1; }
done

# R13-05 (Phase E) synthetic-discipline gate: refuse if a real-device
# hex id snuck into ANY per-version subdirectory. Allow-list:
#   0x0001 (real factory default — never secret)
#   0xa5a2 (synthetic wrap-key canary)
#   0xc995 (synthetic application auth-key)
#   0x0042 (synthetic victim asymmetric key)
# The R10-L2 foil 0xDEAD is uppercase deliberately so this lowercase-
# hex grep skips it. Per the R13-E lock: any hex id outside the allow-
# list is a hard fail, regardless of which subdirectory it appears in.
unexpected_ids=$(grep -rE '0x[0-9a-f]{4}' "${FIXTURES_ROOT}"/v_*/*.txt | grep -vE '0x0001|0xa5a2|0xc995|0x0042' || true)
...
# R13-05 (Phase E): iterate over each per-version subdirectory (i.e.
# every `v_X.Y.Z/` directory) and run the full parser-audit suite
# against each version's transcripts.
shopt -s nullglob
versions=("${FIXTURES_ROOT}"/v_*/)
if [ "${#versions[@]}" -eq 0 ]; then
    echo "FAIL: no version subdirectories in ${FIXTURES_ROOT}" >&2
    exit 1
fi

PARSERS_VERIFIED_TOTAL=0
TRANSCRIPTS_USED_TOTAL=0
...
for version_dir in "${versions[@]}"; do
    ...
    FIXTURES="${version_dir%/}"
    echo "=== Auditing fixtures for ${version_label} ==="

    # Per-version sanity-check: all 6 canonical fixtures must exist.
    for f in yubihsm-shell-list-objects.txt yubihsm-setup-ksp.txt \
        ...
        [ -r "${FIXTURES}/${f}" ] || { echo "FAIL [${version_label}]: missing fixture: ${f}" >&2; exit 1; }
    done
    ...
    if ! printf '%s' "${grep_line}" | grep -qF "grep -c '^id:'"; then
        fail "${version_label}/stage_legacy_setup.sh:173" "n_obj grep is not anchored on '^id:' — production source drifted: '${grep_line}'"
    fi
    n_obj=$(grep -c '^id:' "${FIXTURES}/yubihsm-shell-list-objects.txt")
    if [ "${n_obj}" != "4" ]; then
        fail "${version_label}/stage_legacy_setup.sh:173" "expected n_obj=4 (1 default-auth + 1 wrap + 1 app-auth + 1 R10-L2 foil), got ${n_obj}"
    fi
    ...
    if [ -z "${awk_prog_stage_wrap}" ]; then
        fail "${version_label}/stage_legacy_setup.sh:205-206" "could not extract wrap-key awk program from production source"
    fi
    ...
    if ! printf '%s' "${awk_prog_stage_wrap}" | grep -qF '$4 == "wrap-key"'; then
        fail "${version_label}/stage_legacy_setup.sh:205-206" "R10-L2 regression: production awk lost the column-anchored '\$4 == \"wrap-key\"' predicate; got: ${awk_prog_stage_wrap}"
    fi
    got_wrap_id=$(awk -F'[, ]+' "${awk_prog_stage_wrap}" \
                  "${FIXTURES}/yubihsm-shell-list-objects.txt")
    if [ "${got_wrap_id}" != "0xa5a2" ]; then
        fail "${version_label}/stage_legacy_setup.sh:205-206" "wrap-key extractor diverged from fixture; expected 0xa5a2, got '${got_wrap_id}' (awk: ${awk_prog_stage_wrap})"
    fi
    # Negative-grep: ensure the foil's id 0xDEAD was NOT captured.
    if [ "${got_wrap_id}" = "0xDEAD" ] || [ "${got_wrap_id}" = "0xdead" ]; then
        fail "${version_label}/stage_legacy_setup.sh:205-206" "R10-L2 regression: wrap-key extractor captured foil auth-key id 0xDEAD"
    fi
    PARSERS_VERIFIED=$((PARSERS_VERIFIED + 1))
    ...
    if [ -z "${awk_prog_stage_auth}" ]; then
        fail "${version_label}/stage_legacy_setup.sh:208-209" "could not extract auth-key awk program from production source"
    fi
    # shellcheck disable=SC2016  # `$4` is the literal awk-field reference we're searching for
    if ! printf '%s' "${awk_prog_stage_auth}" | grep -qF '$4 == "authentication-key"'; then
        fail "${version_label}/stage_legacy_setup.sh:208-209" "R10-L2 regression: production auth-key awk lost the column-anchored predicate; got: ${awk_prog_stage_auth}"
    fi
    got_app_auth_id=$(awk -F'[, ]+' "${awk_prog_stage_auth}" \
                      "${FIXTURES}/yubihsm-shell-list-objects.txt")
    if [ "${got_app_auth_id}" != "0xc995" ]; then
        fail "${version_label}/stage_legacy_setup.sh:208-209" "app-auth extractor diverged; expected 0xc995, got '${got_app_auth_id}' (awk: ${awk_prog_stage_auth})"
    fi
    PARSERS_VERIFIED=$((PARSERS_VERIFIED + 1))
    ...
    if [ "${n_shares}" != "3" ]; then
        fail "${version_label}/_extract_shares.sh:12-15" "expected 3 shares (2-of-3 fixture), got ${n_shares} (extract rc=${extract_rc})"
    fi
    # Each output line must match the canonical share shape.
    while IFS= read -r line; do
        if ! printf '%s' "${line}" | grep -qE '^[0-9]{1,2}-[0-9]{1,3}-[A-Za-z0-9+/]{70}$'; then
            fail "${version_label}/_extract_shares.sh:12-15" "non-canonical share line emitted: '${line}'"
        fi
    done < "${SHARES_OUT}"
    # Negative-grep: ensure the noise line 00-00-DDD... was REJECTED.
    if grep -q '^00-' "${SHARES_OUT}"; then
        fail "${version_label}/_extract_shares.sh:12-15" "noise line 00-00-... leaked through bounds filter"
    fi
    # Positive-grep: ensure each expected share is present.
    for prefix in "2-1-" "2-2-" "2-3-"; do
        if ! grep -q "^${prefix}" "${SHARES_OUT}"; then
            fail "${version_label}/_extract_shares.sh:12-15" "expected share prefix ${prefix} missing from extract"
        fi
    done
    PARSERS_VERIFIED=$((PARSERS_VERIFIED + 1))
    ...
    if [ -z "${awk_prog_verify_wrap}" ]; then
        fail "${version_label}/verify_roundtrip.sh:69-71" "could not extract wrap-key awk program from verify source"
    fi
    # shellcheck disable=SC2016  # `$4` is the literal awk-field reference we're searching for
    if ! printf '%s' "${awk_prog_verify_wrap}" | grep -qF '$4 == "wrap-key"'; then
        fail "${version_label}/verify_roundtrip.sh:69-71" "R10-L2 regression: verify's wrap-key awk lost the column-anchored predicate; got: ${awk_prog_verify_wrap}"
    fi
    ...
    if [ "${N_WRAP}" != "1" ]; then
        fail "${version_label}/verify_roundtrip.sh:69-71" "expected exactly 1 wrap-key in fixture, got N_WRAP=${N_WRAP} (${ALL_WRAP_IDS})"
    fi
    if [ "${ALL_WRAP_IDS}" != "0xa5a2" ]; then
        fail "${version_label}/verify_roundtrip.sh:69-71" "ALL_WRAP_IDS extractor diverged; expected 0xa5a2, got '${ALL_WRAP_IDS}' (awk: ${awk_prog_verify_wrap})"
    fi
    PARSERS_VERIFIED=$((PARSERS_VERIFIED + 1))
    ...
    if ! grep -q 'Object imported as' "${OK_LOG}"; then
        fail "${version_label}/verify_roundtrip.sh:96-104" "put-wrapped-ok fixture missing 'Object imported as' marker"
    fi
    if ! grep -qE 'Unable to import|Wrong CCM|invalid wrap-key' "${FAIL_LOG}"; then
        fail "${version_label}/verify_roundtrip.sh:96-104" "put-wrapped-authfail fixture missing CCM-failure marker"
    fi
    if grep -q 'Object imported as' "${FAIL_LOG}"; then
        fail "${version_label}/verify_roundtrip.sh:96-104" "put-wrapped-authfail fixture wrongly contains success marker"
    fi
    PARSERS_VERIFIED=$((PARSERS_VERIFIED + 1))
    ...
    if ! grep -q -- '-----BEGIN PUBLIC KEY-----' "${PEMTXT}"; then
        fail "${version_label}/verify_roundtrip.sh:107" "get-public-key fixture missing PEM BEGIN marker"
    fi
    if ! grep -q -- '-----END PUBLIC KEY-----' "${PEMTXT}"; then
        fail "${version_label}/verify_roundtrip.sh:107" "get-public-key fixture missing PEM END marker"
    fi
    PARSERS_VERIFIED=$((PARSERS_VERIFIED + 1))
    ...
    [ -n "${pat1}" ] || fail "${version_label}/drive_manager.py:361" "could not extract p.expect pattern from production"
    grep -qE -- "${pat1}" "${PROMPTS}" \
        || fail "${version_label}/drive_manager.py:361" "extracted production regex did not match any prompt-fixture line: ${pat1}"

    # drive_manager.py:416 — expect_exact "Re-create from shares?".
    pat2=$(extract_pyexpect 'Re-create from shares?')
    [ -n "${pat2}" ] || fail "${version_label}/drive_manager.py:416" "could not extract p.expect_exact pattern from production"
    grep -qF -- "${pat2}" "${PROMPTS}" \
        || fail "${version_label}/drive_manager.py:416" "extracted literal '${pat2}' missing from prompt fixture"

    # drive_manager.py:429 — Press-any-key recreate prompt.
    pat3=$(extract_pyexpect 'Press any key to recreate wrap key from shares')
    [ -n "${pat3}" ] || fail "${version_label}/drive_manager.py:429" "could not extract p.expect pattern from production"
    grep -qE -- "${pat3}" "${PROMPTS}" \
        || fail "${version_label}/drive_manager.py:429" "extracted regex did not match: ${pat3}"

    # drive_manager.py:433 — share-count prompt.
    pat4=$(extract_pyexpect 'Enter the number of shares to re-create the AES wrap key')
    [ -n "${pat4}" ] || fail "${version_label}/drive_manager.py:433" "could not extract p.expect pattern from production"
    grep -qE -- "${pat4}" "${PROMPTS}" \
        || fail "${version_label}/drive_manager.py:433" "extracted regex did not match: ${pat4}"
    ...
    grep -qF 'p.expect(rf"Enter share number {i}' "${DRIVER_PY}" \
        || fail "${version_label}/drive_manager.py:438" "share-entry f-string pattern drifted in production"
    grep -qE 'Enter share number 1' "${PROMPTS}" \
        || fail "${version_label}/drive_manager.py:438" "fixture missing 'Enter share number 1' line"

    # drive_manager.py:454 — object-label prompt.
    pat6=$(extract_pyexpect 'Enter object label')
    [ -n "${pat6}" ] || fail "${version_label}/drive_manager.py:454" "could not extract p.expect pattern from production"
    grep -qE -- "${pat6}" "${PROMPTS}" \
        || fail "${version_label}/drive_manager.py:454" "extracted regex did not match: ${pat6}"

    # drive_manager.py:459 — metadata-confirmation prompt.
    pat7=$(extract_pyexpect 'Import wrap key with')
    [ -n "${pat7}" ] || fail "${version_label}/drive_manager.py:459" "could not extract p.expect pattern from production"
    grep -qE -- "${pat7}" "${PROMPTS}" \
        || fail "${version_label}/drive_manager.py:459" "extracted regex did not match: ${pat7}"
    ...
    grep -qF '[Ii]mported wrap key with [Ii][Dd][:' "${DRIVER_PY}" \
        || fail "${version_label}/drive_manager.py:474" "case-insensitive Imported regex drifted in production"
    grep -qE '[Ii]mported wrap key with [Ii][Dd][:[:space:]=]' "${PROMPTS}" \
        || fail "${version_label}/drive_manager.py:474" "fixture missing 'Imported wrap key with ID' success line"

    # drive_manager.py:479 — id-capture regex against the same line.
    grep -qF '(0x[0-9a-fA-F]{1,4})' "${DRIVER_PY}" \
        || fail "${version_label}/drive_manager.py:479" "id-capture regex drifted in production"
    grep -qE 'Imported wrap key with ID 0x[0-9a-fA-F]{1,4}' "${PROMPTS}" \
        || fail "${version_label}/drive_manager.py:479" "fixture success line is not id-capturable"

    PARSERS_VERIFIED=$((PARSERS_VERIFIED + 1))

    # Track fixtures used (one per .txt file) for this version.
    TRANSCRIPTS_USED=$(find "${FIXTURES}" -maxdepth 1 -name '*.txt' -type f | wc -l)

    echo "  [${version_label}] ${PARSERS_VERIFIED} parser sites verified against ${TRANSCRIPTS_USED} transcripts"
    PARSERS_VERIFIED_TOTAL=$((PARSERS_VERIFIED_TOTAL + PARSERS_VERIFIED))
    TRANSCRIPTS_USED_TOTAL=$((TRANSCRIPTS_USED_TOTAL + TRANSCRIPTS_USED))
done

echo "PASS: test_transcript_parity.sh (${PARSERS_VERIFIED_TOTAL} site*version checks across ${#versions[@]} version subdirectories, ${TRANSCRIPTS_USED_TOTAL} transcripts total)"
